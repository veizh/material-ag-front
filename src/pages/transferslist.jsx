...
function tableLine(data,i,allInterventions,Navigate){
    ...
    return(
        <tr key={i} onClick={()=>Navigate("/transferTicket/"+data._id)}>
            <td className="groupName">{data && data.sender[0].contractNumber+" " }<b>{ data&& data.sender[0].groupName + " "}   </b>{data && data.sender[0].clientName} </td>
            <td className="groupName">{data && data.recipient[0].contractNumber+ " "}<b>{ data&& data.recipient[0].groupName + " "}  </b>{data && data.recipient[0].clientName}</td>
            <td className="nowrap">{data.date?formateDate(data.date):"Non Renseigné"}</td>
